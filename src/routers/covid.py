...
async def get_us_data(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching US COVID-19 data: {str(e)}")
...
async def get_by_jurisdiction_and_month(
    ...
    try:
        logger.info(f"/US/aggregate/ params: jurisdiction_residence_name='{jurisdiction_residence_name}', month_name='{month_name}', limit={limit}, offset={offset}")
        stmt = (
            select(gold_fact_covid_deaths)
            .where(
                (gold_fact_covid_deaths.JURISDICTION_RESIDENCE_NAME == jurisdiction_residence_name) &
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching COVID-19 data: {str(e)}")

@router.get("/US/{jurisdiction_residence_name}", response_model=List[gold_fact_covid_deaths])
async def get_us_by_jurisdiction(
    ...
    try:
        ...
        statement = select(gold_fact_covid_deaths).where(
            gold_fact_covid_deaths.JURISDICTION_RESIDENCE_NAME == jurisdiction_residence_name
        ).offset(offset).limit(limit)
        us_data = session.exec(statement).all()
        if not us_data:
            logger.info(f"No data found for jurisdiction name: {jurisdiction_residence_name}")
            raise HTTPException(status_code=404, detail="No data found for jurisdiction name")
        return us_data
    except Exception as e:
        logger.error(f"Error fetching US COVID-19 data: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error fetching US COVID-19 data: {str(e)}")
...
async def delete_us_record(covid_deaths_key: int, session: Session = Depends(get_session)):
